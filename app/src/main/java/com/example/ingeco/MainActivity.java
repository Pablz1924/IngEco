...
public class MainActivity extends AppCompatActivity {
    ...
    public void calcular(View view){
        ...
        if (n < 6 || n > 24){ etxtTiempo.setError("Valores fuera del intervalo");
            return;
        }
        ...
        for (int i = 0; i < n; i++){
            ...
            TextView new_col1 = newTextView(Integer.toString(i+1),"#FF000000");
            newRow.addView(new_col1);
            TextView new_col2 = newTextView(df.format(A),"#FF000000");
            newRow.addView(new_col2);
            ...
            if (i == 0){
                ...
                TextView PInteres = newTextView(df.format(Interes),"#FF000000");
                newRow.addView(PInteres);
                TextView PAmortizacion = newTextView(df.format(Amortizacion),"#FF000000");
                newRow.addView(PAmortizacion);
                TextView PSaldo = newTextView(df.format(Saldo),"#FF000000");
                newRow.addView(PSaldo);
            }
            else{
                Interes = Saldo*TEM;
                Amortizacion = A-Interes;
